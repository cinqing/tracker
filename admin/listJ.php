...
$end=	date("Y-m-d");
	...
	//显示数据
	$query = "SELECT * FROM jingjia where time>'".$star."' and time<'".$end." 23:59:59' ".$websql." order by id desc ";
	//echo $query;
...
<div class="text-c">	网站:
		...
		<select class="select" id="web" name="">
			...
			<?php
			$r = "SELECT distinct(web) FROM jingjia  order by id desc ";
...
$(function(){
	$('.table-sort').dataTable({
		...
		"aoColumnDefs": [
		 ...
		 {"orderable":false,"aTargets":[1,3,2,4,5,6,7,8,9]}// 制定列不参与排序
	...
	$("#bth").click(function(){
		...
		window.location.href="listJ.php?star="+$("#datemin").val()+"&end="+$("#datemax").val()+"&web="+$("#web").val();
...
function xiazai(){
	...
	window.location.href="testJ.php?star="+$("#datemin").val()+"&end="+$("#datemax").val()+"&web="+$("#web").val();
